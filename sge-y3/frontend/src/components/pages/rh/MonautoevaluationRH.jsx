...
function MonautoevaluationRH() {
  ...
  useEffect(() => {
    ...
    async function loadEvaluation() {
      try {
        setIsLoading(true);
        setErrorMessage("");
        const response = await getRhSelfEvaluation();
        ...
        if (!cancelled) {
          setErrorMessage(error.message || "Chargement de l'auto-evaluation RH impossible.");
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  async function persistSections(nextSections = sections, successMessage = "Auto-evaluation RH enregistree.") {
    try {
      setIsSaving(true);
      setFeedbackMessage("");
      const response = await saveRhSelfEvaluation({
  ...
  async function handleSaveAndContinue() {
    await persistSections(sections, "Auto-evaluation RH enregistree.");
    goToStep(1);
  }

  async function handleSubmit() {
    const savedEvaluation = await persistSections(sections, "Auto-evaluation RH prete pour soumission.");
    ...
    try {
      ...
      const response = await submitRhSelfEvaluation();
      setEvaluationData(response);
      setSections(response.evaluation.sections || []);
      setFeedbackTone("success");
      setFeedbackMessage(response.message || "Auto-evaluation RH soumise aux associes.");
    } catch (error) {
      setFeedbackTone("error");
  ...
  if (isLoading) {
    return (
      <section className="rounded-md bg-white p-5 text-sm font-semibold text-slate-500 shadow-sm">
        Chargement de l'auto-evaluation RH...
      </section>
    );
  }
      ...
      <section className="grid grid-cols-1 gap-4 xl:grid-cols-12">
        <div className="space-y-3 xl:col-span-5">
          <article className="rounded-md bg-white p-4 shadow-sm">
            ...
            <h2 className="mt-1 text-2xl font-black text-[#0F3A63]">Auto-evaluation RH</h2>
            <p className="mt-1 text-sm font-semibold text-slate-500">
              {evaluationData.rh.name} - {evaluationData.rh.department}
            </p>
          ...
          <article className="rounded-md bg-white p-4 shadow-sm">
            ...
            <div className="space-y-3 text-xs font-semibold text-[#0F3A63]">
              <div className="flex items-start gap-3">
                ...
                <p>Vous saisissez votre auto-evaluation RH</p>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p>Soumission aux associes</p>
                  <p className="text-[11px] text-slate-400">Apres completion de toutes les sections de la matrice RH</p>
                </div>
              </div>
              {associateRecipients.map((recipient, index) => (
                <div key={recipient.id} className="flex items-start gap-3">
                  ...
                  <p>
                    Associe destinataire - {recipient.name} ({recipient.department || recipient.grade})
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  disabled={isSaving || isSubmitting || evaluationData.evaluation.status === "Soumis aux Associes"}
                  className="rounded-md bg-[#79B742] px-8 py-2 text-xs font-semibold text-white disabled:opacity-70"
                >
                  {isSubmitting ? "Soumission..." : "Soumettre aux associes"}
                </button>
              </div>
            </div>
          </article>
