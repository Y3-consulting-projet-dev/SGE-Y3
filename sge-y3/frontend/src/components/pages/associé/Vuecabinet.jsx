...
import { getCommitteeOverview, saveCommitteeDecision } from "@/lib/committee";
...
const sideMenu = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: "committee", label: "Comite d'evaluation", icon: UsersRound },
    ],
  },
  {
    title: "Managers",
    items: [
      { key: "autoeval-managers", label: "Auto-eval Managers", icon: UsersRound },
      { key: "autoeval-support", label: "Auto-eval Support", icon: UsersRound },
    ],
  },
  {
    title: "Reporting",
...
const fallbackDepartmentScores = [
...
function Vuecabinet({ onLogout, onUserUpdate, user }) {
  ...
  useEffect(() => {
    ...
    getCommitteeOverview()
      ...
      .catch((error) => {
        ...
        setOverviewStatus({
          loading: false,
          error: error.message || "Chargement de la vue cabinet impossible.",
        });
      });
  ...
  const pageTitle = useMemo(() => {
    ...
    if (activeSection === "autoeval-support") return "Auto-eval Support";
    if (activeSection === "history") return "Historique & analytics";
    if (activeSection === "profile") return "Mon profil";
    return "Vue cabinet";
  }, [activeSection]);

  const summaryCards = overviewData?.summary_cards?.length ? overviewData.summary_cards : fallbackSummaryCards;
  const decisionSplit = overviewData?.decision_split?.length ? overviewData.decision_split : fallbackDecisionSplit;
  const urgentDecisions = overviewData?.urgent_decisions?.length ? overviewData.urgent_decisions : fallbackUrgentDecisions;
  const departmentScores = overviewData?.department_scores?.length ? overviewData.department_scores : fallbackDepartmentScores;
  const decisionSplitTotal =
    typeof overviewData?.decision_split_total === "number"
      ? overviewData.decision_split_total
      : fallbackDecisionSplit.reduce((total, item) => total + item.count, 0);
  const synthesesReceived =
    summaryCards.find((item) => item.title === "Syntheses recues RH")?.value || "11";
          ...
          {activeSection === "vue-cabinet" ? (
            <>
              <header className="mb-8 flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {displayName} - {user?.grade} - {overviewData?.header_subtitle || "Cycle 2025-2026 - Synthese globale"}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#1F4A72] text-xs font-bold text-white">
                      {initials}
                    </div>
              ...
              {overviewStatus.error ? (
                <section className="mb-5 rounded-xl border border-[#F0C9C9] bg-[#FFF5F5] px-4 py-3 text-sm font-semibold text-[#A33A3A]">
                  {overviewStatus.error}
                </section>
              ) : null}
                ...
                {summaryCards.map((card) => (
                  <article key={card.title} className="rounded-lg bg-[#0D496A] p-4 text-white">
                    ...
                    <p className="mt-2 text-2xl font-extrabold leading-none">
                      {overviewStatus.loading ? "..." : card.value}
                    </p>
              ...
              <section className="mb-8 grid grid-cols-1 gap-5 xl:grid-cols-2">
                <article className="rounded-xl bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-5 text-xs font-semibold text-slate-500">
                    {decisionSplitTotal} decisions prises sur {synthesesReceived} syntheses
                  </p>
                </article>
                    ...
                    {urgentDecisions.length ? (
                      urgentDecisions.map((item) => (
                        <div
                          ...
                          className="flex flex-wrap items-center justify-between gap-4 rounded-lg bg-slate-50 p-3"
                        >
                      ...
                      <div className="rounded-lg bg-slate-50 px-4 py-5 text-sm font-semibold text-slate-500">
                        Aucune decision urgente pour le moment.
                      </div>
                    )}
                  </div>
                </article>
