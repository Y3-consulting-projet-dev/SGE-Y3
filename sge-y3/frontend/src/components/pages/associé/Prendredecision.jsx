...
const scoreItems = [
  ...
  { label: "Competences techniques", score: "3.9", width: "69%" },
      ...
      <section className="grid grid-cols-1 gap-4 xl:grid-cols-[1.45fr_1fr]">
        <article className="rounded-lg bg-white p-4">
          ...
          <div className="mb-3 rounded-md bg-[#E3E3E3] px-3 py-2.5">
            ...
            <p className="text-sm text-[#6B7078]">"Tres bonne maitrise du portefeuille clients, leadership reconnu par l'equipe."</p>
          </div>

          <div className="mb-4 flex items-center justify-between">
            <div>
              <p className="text-base font-bold text-[#78B843]">Auto-evaluation du Manager</p>
          ...
          <div className="mb-4 space-y-5">
            {selfEvalRows.map((row) => (
              <div key={row.label} className="flex items-end justify-between">
                <p className="text-[30px] font-black leading-none text-[#0F3A63]">{row.label}</p>
                <p className="text-xs font-bold text-[#78B843]">{row.score}</p>
              </div>
            ))}
            <p className="text-[10px] text-slate-500">
              "Je souhaite prendre plus de responsabilites sur des mandats d'envergure. Pret pour evoluer."
            </p>
          </div>

          <div>
            <p className="mb-2 text-sm font-bold text-[#0F3A63]">Annotation Associe (Facultative)</p>
            <textarea
              value={comment}
              onChange={(event) => setComment(event.target.value)}
              placeholder="Commenter l'auto-evaluation du manager..."
        ...
        <aside className="space-y-3">
          <section className="rounded-lg bg-white p-4">
            <p className="text-lg font-black text-[#0F3A63]">Decision RH</p>

            <div className="mt-3 space-y-3">
              <div>
                <p className="mb-1 text-sm font-bold text-[#1E2B3A]">Type de decision</p>
              ...
              <div>
                ...
                <textarea
                  value={comment}
                  onChange={(event) => setComment(event.target.value)}
                  placeholder="Justification de la decision, axes de developpement..."
              ...
              <div>
                <p className="mb-2 text-sm font-bold text-[#1E2B3A]">Envoyer la notification automatique a la cloture</p>
          ...
          <section className="rounded-lg bg-white p-4">
            <p className="mb-3 text-xl font-black text-[#0F3A63]">Contexte - historique decisions</p>
